...
class LinmosParsetSummary(NamedTuple):
    """Summary of important components of the linmos parset"""

    parset_path: Path
    """Path to the generated linmos parset file"""
    weights_text_list: Tuple[Path, ...]
    """The set of paths to the weights text files provided to linmos"""
    image_paths: Tuple[Path, ...]
    """The set of paths to the images provided to linmos"""
...
def generate_linmos_parameter_set(
    ...
    Returns:
        LinmostParsetSummary: Import components of the parset, including Path to the output parset file.
    """
...
def linmos_images(
    ...
    Args:
        ...
        cleanup (bool, optional): If True, remove files created by linmos that are no longer needed (e.g. the weights text files). Defaults to False
    ...
    if cleanup:
        logger.info("Removing linmos files no longer needed")
        [
            weight_text.unlink()
            for weight_text in linmos_parset_summary.weights_text_list
        ]
