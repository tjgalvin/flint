...
def get_common_beam(image_paths: Collection[Path], cutoff: Optional[float]=None) -> BeamShape:
...
def get_parser() -> ArgumentParser:
    ...
    convol_parser = subparsers.add_parser('convol', help='Convol images to a common resolution')
    
    convol_parser.add_argument(
        'images', type=Path, nargs='+', help='The images that will be convolved to a common resolution'
    )
    convol_parser.add_argument(
        '--cutoff', type=float, default=None, help='Beams whose major-axis are larger then this (in arcseconds) are ignored from the calculation of the optimal beam.'
    )
    convol_parser.add_argument('--convol-suffix', type=str, default='conv', help='The suffix added to convolved images. ')

    maxbeam_parser = subparsers.add_parser('maxbeam', help='Find the optimal beam size for a set of images.')
