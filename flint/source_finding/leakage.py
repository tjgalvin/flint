...
class StokesData:
    ...
    def __init__(
        ...
        if catalogue_i.lower().endswith(".fits"):
            table_format = "fits"
        elif catalogue_i.lower().endswith(".vot") or catalogue_i.lower().endswith(
            ".xml"
        ):
        ...
        abs_clipped = sigma_clip(
            np.abs(self.data), sigma=5, maxiters=3, masked=False, axis=(0, 1)
        )
        self.data_clipped = self.data.copy()
        self.data_clipped[np.isnan(abs_clipped)] = np.nan
        self.noise = np.nanstd(self.data_clipped)
        logger.info("Stokes V sigma-clipped map stddev.: {}".format(self.noise))
        try:
            ...
            self.coords = SkyCoord(
                ra=self.catalogue_i[ra_key], dec=self.catalogue_i[dec_key]
            )
    ...
    def get_isolated(self, zone: float = 0.0055):
        ...
        """
        idx, seps, _ = self.coords.match_to_catalog_sky(self.coords, nthneighbor=2)
        isolated = np.where(seps.value > zone)[0]
        logger.info(
            "Isolated fraction: {}".format(len(isolated) / len(self.catalogue_i))
        )
    ...
    def get_leakage(
        ...
        for i in range(len(self.coords)):
            ...
            if (
                self.catalogue_i[self.peak_flux][i] / self.catalogue_i[local_rms][i]
                > sigma
            ):
    ...
    def get_table_leakage(
        ...
        self.catalogue_i = self.catalogue_i[
            np.where(np.isfinite(self.catalogue_i[polname + "_leakage"]))[0]
        ]

        if outname is not None:
            self.catalogue_i.write(
                outname, format="ascii", delimiter=",", overwrite=True
            )
...
def extract_leakage(
    ...
    data.get_isolated(zone=exclusion_zone / 3600.0)
    data.get_compact(int_flux=int_flux_key, peak_flux=peak_flux_key, ratio=(0.5, 2.0))
    data.get_xycoords()
    data.get_leakage(local_rms=local_rms_key, sigma=snr)
    data.get_table_leakage(
        outname=image_p.replace(".fits", "") + f"_{pol}leakage.csv", pol=pol
    )
...
"""
    ...
    ps.add_argument(
        "--ra_key", default="ra", type=str, help="RA column name. [Default 'ra'']"
    )
    ps.add_argument(
        "--dec_key", default="dec", type=str, help="DEC column name. [Default 'dec']"
    )
    ps.add_argument(
        "--int_flux_key",
        default="int_flux",
